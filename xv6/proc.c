...
found:
	...
	p->run_time = 0;             
		...
		p->lrun_time = 0;			  
		p->lwtime = 0;			     
		p->mlfq_proc.pid = p->pid;
		p->mlfq_proc.runtime = 0;
		p->mlfq_proc.num_run = 0;
		p->mlfq_proc.current_queue = 0;
		for(int i = 0; i < 5; i++){
			p->mlfq_proc.ticks[i] = 0;
		}
	#endif
	...
	for(;;){
		...
		acquire(&ptable.lock);
			...
			for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
				...
				if(minP != 0){
					// find the process with the lowest creation time
					if(p->start_time < minP->start_time)
						minP = p;
				}
				else
					minP = p;
			...
			if(lowP != 0){
				...
				lowP->state = RUNNING;
				lowP->num_run++;
				// cprintf("process name: %s, pid: %d, priority: %d\n", lowP->name, lowP->pid, lowP->priority);
			...
			if(p != 0){
				...
				p->state = RUNNING;
				p->mlfq_proc.num_run++;
				// cprintf("process name: %s, pid: %d, queue no: %d\n", p->name, p->pid, p->priority); //priority is queue level
				swtch(&(c->scheduler), p->context);
				switchkvm();
...
yield(void)
{
	...
	#else
	#ifdef MLFQ
		if(myproc()->lrun_time >= prq[myproc()->priority].max_run_time){
			if(myproc()->priority != (5 -1)){
				myproc()->priority++;
				cprintf("Demotion of pid %d to queue %d\n", myproc()->pid, myproc()->priority);
			}	
			myproc()->lrun_time = myproc()->lwtime = 0;
			myproc()->mlfq_proc.current_queue = myproc()->priority;
			myproc()->state = RUNNABLE;
        	pq_push(myproc());
			sched();
		}
			...
			if(toPreempt){
				myproc()->lwtime = myproc()->lrun_time = 0;
				myproc()->state = RUNNABLE;
				pq_push(myproc());
				sched();
			}
...
check_starvation(){
	...
	for(int i = 1; i < 5; i++){
		...
		while(prq[i].front != -1 && p->state == RUNNABLE && p->lwtime >= prq[i].max_wtime){
			pq_pop(p);
			p->priority--;
			cprintf("promotion of pid %d to queue %d\n", p->pid, p->priority);
			p->mlfq_proc.current_queue = p->priority;
			p->lrun_time = p->lwtime = 0;
			pq_push(p);
			if(prq[i].front != -1)
				p = prq[i].proc[prq[i].front];
		}
